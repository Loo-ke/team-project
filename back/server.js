...
app.get("/mypageInfo", function (req, res) {
  ...
  var Reviewquery =
    "select distinct* from USER left join SHOP USING(user_id) where user_id ='" +
    req.session.user_id +
    "';"; //using
  var Commentquery =
...
//   })
  ...
  // let addsql = await conn.query(
  //   `insert into Shop (user_id, shop_name, shop_addr, telno, latitude, longitude, shop_url, Field)
  // values (${req.session.user_id}, "${req.body.place_name}", "${req.body.place_address}", 
  // "${req.body.place_phone}", "${req.body.place_x}", "${req.body.place_y}", 
  // "${req.body.place_url}", "${req.body.place_text}")`,
  //   addsql,
  //   function (err, rows) {
